...
To use the SDK, you need to get a token, and you can check the token by registering as a member of [DS2.ai](http://ds2.ai/). After registering the card on the desktop application, you can use the token.  
...
Then you can use all functions in [DS2.ai](http://ds2.ai) SDK.
...
```python
ds2.traintrain(data_file, training_method, value_for_predict, option="accuracy", frame=60):
```
